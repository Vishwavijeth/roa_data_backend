...
def transaction_specialist_dashboard(
    ...
    try:
        ...
        if from_date:
            query += " AND be.closed_date::date >= :from_date"
            params["from_date"] = from_date

        if to_date:
            query += " AND be.closed_date::date <= :to_date"
            params["to_date"] = to_date

        if state:
            cleaned_states = [s.strip() for s in state if s and s.strip()]
            if cleaned_states:
                query += " AND LOWER(COALESCE(be.state, '')) = ANY(:states)"
                params["states"] = [s.lower() for s in cleaned_states]
        ...
        state_rows = db.execute(text(states_query)).scalars().all()
