...
# 1. Removed grouping of entire sale_contact table (which was scanning the whole DB).
#    Instead, contact details are retrieved using correlated subqueries in the SELECT clause,
#    which only execute for the filtered rows and leverage indices.
# 2. Kept skyslopefileid as UUID to allow index lookup on sale(saleguid).
BASE_QUERY = """
...
other_income_base AS (
    SELECT
        ...
        -- Parameter values for Other Income
...
SELECT
    ...
    -- SkySlope values (using correlated subqueries for high performance)
...
LEFT JOIN sale s
    ON s.saleguid = cs.skyslopefileid
...
def evaluate_row(row):
    ...
    # Brokerage Engine only parameters
...
# ── Canonical parameter display names (single source of truth) ──────────────
...
def get_reconciliation_transactions(
    ...
    # Parse mismatch parameters
    parsed_mismatch_params = []
    if mismatch_parameter:
        for p in mismatch_parameter:
            for part in p.split(","):
                part_stripped = part.strip().lower().replace(" ", "_")
                if part_stripped:
                    parsed_mismatch_params.append(part_stripped)

    NOT_CANCELLED = "NOT (cs.be_status ILIKE 'cancelled' AND cs.skyslope_status ILIKE ANY (ARRAY['canceled/pend', 'canceled/app']))"

    MISMATCH_SQL_FILTERS = {
        "close_date":         f"(cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_close_date IS NOT NULL AND cs.skyslope_close_date IS NOT NULL AND cs.be_close_date != cs.skyslope_close_date)",
        "sale_price":         f"(cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_sale_price IS NOT NULL AND cs.skyslope_sale_price IS NOT NULL AND cs.be_sale_price != cs.skyslope_sale_price)",
        "listing_price":      f"(cs.source_table = 'brokerage_engine' AND cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_listing_price IS NOT NULL AND cs.skyslope_listing_price IS NOT NULL AND cs.be_listing_price != 0 AND cs.skyslope_listing_price != 0 AND cs.be_listing_price != cs.skyslope_listing_price)",
        "status":             f"(cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND (cs.be_status IS NULL OR cs.skyslope_status IS NULL OR (NOT (cs.be_status ILIKE 'closed' AND cs.skyslope_status ILIKE ANY (ARRAY['archived', 'closed'])) AND NOT (cs.be_status ILIKE cs.skyslope_status) AND NOT (cs.be_status ILIKE 'pending' AND cs.skyslope_status ILIKE 'expired'))))",
        "buyer_name":         f"(cs.source_table = 'brokerage_engine' AND cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_buyer_name IS NOT NULL AND TRIM(cs.be_buyer_name) != '' AND cs.skyslope_buyer_name IS NOT NULL AND TRIM(cs.skyslope_buyer_name) != '' AND LOWER(TRIM(cs.be_buyer_name)) != LOWER(TRIM(cs.skyslope_buyer_name)))",
        "seller_name":        f"(cs.source_table = 'brokerage_engine' AND cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_seller_name IS NOT NULL AND TRIM(cs.be_seller_name) != '' AND cs.skyslope_seller_name IS NOT NULL AND TRIM(cs.skyslope_seller_name) != '' AND LOWER(TRIM(cs.be_seller_name)) != LOWER(TRIM(cs.skyslope_seller_name)))",
        "buying_agent_name":  f"(cs.source_table = 'brokerage_engine' AND cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_buying_agent_name IS NOT NULL AND TRIM(cs.be_buying_agent_name) != '' AND cs.skyslope_buying_agent_name IS NOT NULL AND TRIM(cs.skyslope_buying_agent_name) != '' AND LOWER(TRIM(cs.be_buying_agent_name)) != LOWER(TRIM(cs.skyslope_buying_agent_name)))",
        "title_company":      f"(cs.source_table = 'brokerage_engine' AND cs.saleguid IS NOT NULL AND {NOT_CANCELLED} AND cs.be_title_company IS NOT NULL AND TRIM(cs.be_title_company) != '' AND cs.skyslope_title_company IS NOT NULL AND TRIM(cs.skyslope_title_company) != '' AND LOWER(TRIM(cs.be_title_company)) != LOWER(TRIM(cs.skyslope_title_company)))",
        "gross_commission":   f"""(
    ...
    for row in rows:
        ...
        mismatch_params = [
            param for param, eval_data in eval_result.items()
    ...
    # Fetch no_skyslopefileid counts
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
            SELECT
        ...
        """)
        no_skyslope_counts = cur.fetchone()
