...
from api.listing.commission_advances.utils import CommissionAdvanceStatus, CommissionAdvanceLegalHoldStatus
...
def build_listing_filters(status: Optional[CommissionAdvanceStatus], search: Optional[str]):
    ...
    if search and search.strip():
        ...
        filters.append(or_(CommissionAdvance.agent_name.ilike(f"%{search_term}%"), CommissionAdvance.address.ilike(f"%{search_term}%")))
...
def build_global_outstanding_cte():
    ...
    normal_debt = select(
        ...
        ~exists().where(transaction.ca_id == ca.id, transaction.legal_hold_id.is_not(None)),
    )

    legal_hold_debt = select(
        legal_hold.agent_name.label("agent_name"),
        legal_hold.outstanding_amount.label("outstanding_amount"),
    ).where(
        legal_hold.agent_name.isnot(None),
        func.trim(legal_hold.agent_name) != "",
        legal_hold.status == CommissionAdvanceLegalHoldStatus.ACTIVE.value,
        legal_hold.outstanding_amount > 0,
    )

    debt_components = union_all(normal_debt, legal_hold_debt).subquery("debt_components")

    return select(
        debt_components.c.agent_name,
        func.coalesce(func.sum(debt_components.c.outstanding_amount), 0).label("total_outstanding"),
    ).group_by(debt_components.c.agent_name).cte("global_outstanding")
...
def get_commission_advances_listing(
    ...
    try:
        ...
        filtered_data = select(ca.agent_name, ca.status).where(*filters).cte("filtered_data")

        filtered_agents = select(filtered_data.c.agent_name).where(
            filtered_data.c.agent_name.isnot(None),
            func.trim(filtered_data.c.agent_name) != "",
        ).distinct().cte("filtered_agents")

        total_count = db.scalar(select(func.count()).select_from(filtered_agents)) or 0
        ...
        status_breakdowns = select(
            status_counts.c.agent_name,
            func.jsonb_object_agg(status_counts.c.status, status_counts.c.status_count).label("status_breakdown"),
        ).group_by(status_counts.c.agent_name).cte("status_breakdowns")

        priority_expression = case(
            (filtered_data.c.status == CommissionAdvanceStatus.LEGAL_HOLD.value, 1),
        ...
        agent_status_priority = select(
            filtered_data.c.agent_name,
            func.min(priority_expression).label("status_priority"),
        ).group_by(filtered_data.c.agent_name).cte("agent_status_priority")

        agent_user_status = select(
            beu.display_name.label("agent_name"),
            func.max(beu.agent_status).label("agent_status"),
        ).group_by(beu.display_name).cte("agent_user_status")
        ...
        ).select_from(filtered_agents).outerjoin(
            global_outstanding,
        ...
        status_order = [
            CommissionAdvanceStatus.LEGAL_HOLD.value,
        ...
        for row in rows:
            ...
            ordered_breakdown = {status_name: raw_breakdown[status_name] for status_name in status_order if status_name in raw_breakdown}
        ...
        return PaginationResponseWithCount[Dict[str, Any]](
            success=True,
            data=PaginationData[Dict[str, Any]](total_count=total_count, items=items),
            page=page,
            page_size=page_size,
            count=len(items),
...
@router.get("/detail", response_model=CommissionAdvanceDetailResponse)
def get_commission_advances_detail(
    ...
    try:
        ...
        global_outstanding = db.scalar(
            select(global_outstanding_cte.c.total_outstanding).where(global_outstanding_cte.c.agent_name == agent_name)
        ) or 0

        paginated_ca_ids = select(ca.id).where(
