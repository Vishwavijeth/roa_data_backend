...
def otherincome_transactions(
    ...
    data_query += " ORDER BY income_received_date DESC NULLS LAST"
...
def otherincome_detail(transactionid: str, db: Session = Depends(get_db)):
    raw_conn = db.connection().connection
    cursor = raw_conn.cursor()
    ...
    return {
        ...
        "otherincome": {
            ...
            "agents_identifier": data.get("agents_identifier"),
...
@router.get("/otherincome_transactions/download")
def download_otherincome_transactions(
    from_income_received_date: str = Query(default=None),
    to_income_received_date: str = Query(default=None),
    from_finalized_date: str = Query(default=None),
    to_finalized_date: str = Query(default=None),
    ...
    data_query = "SELECT * " + base_query + " ORDER BY income_received_date DESC NULLS LAST"
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df.to_excel(writer, sheet_name="Other Income Transactions", index=False)
        worksheet = writer.sheets["Other Income Transactions"]
    ...
    filename = "otherincome_transactions_report.xlsx"
    headers = {
