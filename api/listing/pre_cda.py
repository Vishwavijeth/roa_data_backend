        ...
        common_cte = """
            ...
            brokerage_base AS (
                SELECT
                    ...
                    be.sale_price::numeric AS source_amount,
                    be.closed_date::date AS source_closed_date,
                    be.transaction_status AS source_transaction_status,
                    CASE
                    ...
                    END::numeric AS source_gross_commission
                FROM brokerage_engine be
            ...
            other_income_base AS (
                SELECT
                    ...
                    oit.income_received::numeric AS source_amount,
                    oit.income_received_date::date AS source_closed_date,
                    oit.transaction_status AS source_transaction_status,
                    oit.gross_commission::numeric AS source_gross_commission
                FROM otherincome_transactions oit
            ...
            base AS (
                SELECT
                    ...
                    cs.source_closed_date,
                    s.escrowclosingdate::date AS ss_closed_date,
                    cs.source_transaction_status,
                    ...
                    CASE
                        ...
                        WHEN cs.source_amount IS DISTINCT FROM s.saleprice::numeric THEN TRUE
                        ELSE FALSE
                    END AS amount_mismatch,
                    CASE
                        WHEN cs.skyslopefileid IS NULL OR s.saleguid IS NULL THEN TRUE
                        WHEN cs.source_closed_date IS DISTINCT FROM s.escrowclosingdate::date THEN TRUE
                        ELSE FALSE
                    END AS closed_date_mismatch,
                    CASE
                        WHEN cs.skyslopefileid IS NULL OR s.saleguid IS NULL THEN TRUE
                        WHEN LOWER(s.status) = 'expired' THEN NULL
                        WHEN cs.source_transaction_status IS NULL OR s.status IS NULL THEN NULL
                        WHEN LOWER(cs.source_transaction_status) = LOWER(s.status) THEN FALSE
                        WHEN LOWER(cs.source_transaction_status) = 'cancelled'
                             AND LOWER(s.status) IN ('canceled/app', 'canceled/pend')
                        THEN FALSE
                        WHEN LOWER(cs.source_transaction_status) = 'closed'
